# Web Component Template - Complete Example

Production-ready Web Component following all `dev-webcomponent` agent standards.

See [examples/data-card/](examples/data-card/) for the actual runnable files.

---

## Example: `data-card` Component

A reusable card component with customizable theming, dynamic content updates, and full accessibility.

### Directory Structure

```text
landing/src/components/data-card/
├── data-card.js
├── data-card.css
└── data-card.html
```
...
<template id="data-card-template">
  <article class="card">
    ...
    <footer class="card-footer">
      <slot name="footer">
        <button class="card-action" type="button">Action</button>
      </slot>
    </footer>
  </article>
</template>
...
- Named `<slot>` tags for content distribution
...
Key points:
- `:host` for component-level styling and CSS variables
- `::slotted()` for styling slotted content
...
export class DataCard extends HTMLElement {
  ...
  #applyTheme(theme) {
    const colors = { primary: '#2563eb', success: '#10b981', warning: '#f59e0b', danger: '#ef4444' };
  ...
  static get CSS() {
    return `/* paste data-card.css content here */`;
  }
}
...
<data-card id="card-2" title="Custom Content">
  <div slot="content"><p>Any HTML here.</p></div>
  <button slot="footer">Custom Action</button>
</data-card>
...
<script type="module">
  ...
  card.setData({ title: 'Updated', theme: 'success' });

  document.addEventListener('card-action', (e) => console.log(e.detail));
  document.addEventListener('card-closed', (e) => console.log('Closed:', e.detail.cardId));
</script>
```
...
- [ ] `observedAttributes` getter defined
- [ ] All lifecycle hooks implemented
- [ ] Private fields with `#` prefix
- [ ] No blind `innerHTML` rewrites
- [ ] Listeners cleaned up in `disconnectedCallback()`
